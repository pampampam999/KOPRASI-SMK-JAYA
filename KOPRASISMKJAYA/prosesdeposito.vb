Public Class prosesdeposito
    Dim kdanggota As String
    Private Sub prosesdeposito_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        tampil()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Dim sisa As Integer = TextBox4.Text - TextBox3.Text
        MsgBox(sisa)
        Dim simpan As String = "insert into penjualan_anggota  values ('" & TextBox1.Text & "','" & kdanggota & "','" & Today.ToString("yyyy-MM-dd") & "','" & TextBox3.Text & "','DEPOSIT')"
        jalankansql(simpan)
        Dim update As String = "update penjualan_rinci_anggota set status=1,kd_penjualan_anggota='" & kdanggota & "' where status=0"
        jalankansql(update)
        Dim update1 As String = "update anggota set deposito=" & sisa & " where kd_anggota='" & kdanggota & "' "
        jalankansql(update1)
        tampil()
    ...
    Private Sub TextBox2_TextChanged(sender As Object, e As EventArgs) Handles TextBox2.TextChanged
        konek()
        DA = New SqlClient.SqlDataAdapter("select kd_anggota,nm_anggota,deposito from anggota where nm_anggota like '%" & TextBox2.Text & "%' ", conn)
        DS = New DataSet
        DA.Fill(DS, "anggota")
        dgv1.DataSource = DS.Tables("anggota")
    End Sub
